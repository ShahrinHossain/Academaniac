...
def modify_csv(rank, input_file, output_file):
    ...
    with open(input_file, "r", newline='') as infile, open(output_file, "w", newline='') as outfile:
        ...
        for row in reader:
            ...
            second_attribute = mapping.get(row[0], None)  # Get value from mapping or None

            if second_attribute is not None:

                modified_row = [first_attribute, second_attribute, row[1], row[2]]  # Replace GRE column with average
                writer.writerow(modified_row)  # Write row without trailing space
            else:
                print(f"Warning: Skipping row with unknown second attribute: {row[0]}")


# Example usage (replace with your file paths)
modify_csv(44, "work_files/fields_removed.csv", "work_files/final_cgpa.csv")
